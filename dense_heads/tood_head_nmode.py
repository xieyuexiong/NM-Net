...
class TOODHead_nmODE(ATSSHead):
    ...
    def __init__(self,
        ...
        super(TOODHead_nmODE, self).__init__(num_classes, in_channels, **kwargs)
...
def draw_feature_map(features, save_dir = '/root/workspace/mmdetection/feature_map',name = 'aaaaa'):
    i=0
    if isinstance(features,torch.Tensor):
        ...
        for featuremap in features:
            ...
            # heatmap = cv2.resize(heatmap, (512,512))  # 将热力图的大小调整为与原始图像相同
            for heatmap in heatmaps:
                heatmap = cv2.resize(heatmap, (608, 608))  # 将热力图的大小调整为与原始图像相同
                ...
                # superimposed_img = np.concatenate((img.to(torch.device('cpu')), heatmap))
